...
function getUserSex(x) {
    let sex = document.querySelectorAll('[name="sex"]');
    for (let i = 0; i < sex.length; i++) {
        if (sex[i].value == x) sex[i].checked = true;
        if (sex[i].checked) {
            sex = sex[i];
            break;
        }
...
function updateUserData(result) {
    console.log(result);
    if (result==1) M.toast({html: 'Data successfully updated.'});
    else M.toast({html: 'mistake in updating'});
}